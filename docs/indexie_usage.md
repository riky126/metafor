...
async def add_user_optimistically():
    ...
    async with db.users.start_optimistic() as tx:
         ...
         try:
             ...
             # 4. Commit: Persist overlay changes to IndexedDB
             # Temp keys are stripped, real keys generated by DB
             await tx.commit() 
         except Exception:
             # 5. Rollback: Discard overlay changes on failure
             await tx.rollback()
```

This prevents temporary "draft" data from polluting your permanent IndexedDB store while keeping the UI snappy.
